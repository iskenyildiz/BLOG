...
Once you get in, you should check your network settings using "ifconfig" to see if you have the network adapter configured. Then proceed to ping a location to see internet connectivity. Pinging Google dns server's is plenty.
...
A block device is a computer data storage device that supports reading and writing data in fixed-size blocks or sectors. Example block devices are floppy disks, hard drives and optical drives.
